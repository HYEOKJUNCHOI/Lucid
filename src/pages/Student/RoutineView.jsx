...
import { addDailyXP, onRoutineComplete, syncStreakToFirestore, getStreakMultiplier, getRoutineRepeatCount, recordRoutineRepeat, getRoutineDropRate } from '../../services/learningService';
...
// ─── 로딩 멘트 ─────────────────────────────────
const LOADING_MSGS = {
  prepare: ['오늘 볼 코드 준비하는 중...', '루틴 세팅하는 중...', '코드 꺼내오는 중...'],
...
// ═══════════════════════════════════════════════
const RoutineView = ({ teacher, repo, chapters, chapterFilesMap, chaptersLoading, visitedFiles, userData, onBack }) => {
  // ─── 상태 ─────────────────────────────────────
  const [routineItems, setRoutineItems] = useState([]);
  const [currentIdx, setCurrentIdx] = useState(() => {
    try { const saved = JSON.parse(localStorage.getItem('lucid_routine_progress')); return saved?.idx || 0; } catch { return 0; }
  });
  ...
  useEffect(() => {
    ...
    const loadFiles = async () => {
      ...
      } catch (e) { console.warn('루틴 파일 로드 실패:', e); }
  ...
  useEffect(() => {
    ...
    // 저장된 진행 상태 복원
    try {
      const saved = JSON.parse(localStorage.getItem('lucid_routine_progress'));
      const today = new Date().toDateString();
      if (saved && saved.date === today && saved.idx < items.length) {
        // 같은 루틴 아이템인지 확인 (파일 경로 비교)
  ...
  const handleAnswer = async (answer) => {
    ...
    if (isCorrect) {
      ...
      const repeatCount = getRoutineRepeatCount(filePath);
      const { multiplier, fixed } = getRoutineDropRate(repeatCount);
      ...
      const xp = addDailyXP(droppedXP, 'routine', true);
  ...
  const goNextQuestion = () => {
    ...
    if (quizHearts <= (quizFeedback?.correct ? 0 : 1) || quizIdx + 1 >= quizQuestions.length) {
      // 반복 횟수 기록 (드랍률 계산용)
      const filePath = routineItems[currentIdx]?.file?.path || '';
      if (filePath) recordRoutineRepeat(filePath);
      setQuizDone(true);
      setQuizFeedback(null);
    } else {
  ...
  const goNextItem = () => {
    if (currentIdx + 1 >= routineItems.length) {
      localStorage.removeItem('lucid_routine_progress');
      setPhase('done');
    } else {
      const nextIdx = currentIdx + 1;
      localStorage.setItem('lucid_routine_progress', JSON.stringify({
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full animate-fade-in-up px-4">
        <div className="w-full max-w-md text-center">
          ...
          <h2 className="text-2xl font-black text-white mb-2">루틴 완료!</h2>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="shrink-0 px-4 py-2.5 border-b border-white/[0.06] flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => setExitConfirm(true)} className="text-gray-400 hover:text-white transition p-1" title="루틴 나가기">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
          <span className="text-sm font-bold text-white">오늘의 루틴</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setExitConfirm(false)}>
          <div className="bg-[#1a1a1a] border border-white/[0.08] rounded-2xl p-6 w-[340px] shadow-[0_16px_64px_rgba(0,0,0,0.5)]" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-sm font-bold text-white">루틴 종료</p>
                <p className="text-xs text-gray-500">진행 상태가 초기화됩니다</p>
              </div>
            </div>
            <p className="text-sm text-gray-300 mb-6">루틴을 정말 종료하시겠습니까?</p>
            <div className="flex gap-2">
